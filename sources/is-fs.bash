...
function is_fs_invoke {
	...
	# failed paths are output to a fixed path because there is no simple way to separate the failed paths from other stdout and stderr output when using sudo in in-no tty mode, as sudo will be using stderr for its own output, and get-owner.bash outputs to stdout
	...
	local command="$1" failures="$XDG_CACHE_HOME/is-fs-failed-paths" # this is serial
	__print_line >"$failures"                                        # reset
	__try {fs_status} -- \
		sudo-helper --inherit --elevated="$option_elevated" --elevate="$option_elevate" --user="$option_user" --group="$option_group" --reason="$option_reason" -- \
