...
class UsageError extends HelpError {
	override help = heredoc`
		USAGE:
		\`echo-regexp.ts [-]<flags> <find-regexp> ...[<replacement>] <input.txt\`
	`
}
...
async function main(...args: string[]) {
	if (args.length < 2) {
		throw new UsageError('--help=Expected <flags> and <find-regexp>.')
	}

	const flags = new Set((args[0] || '').replace(/^-/, '').split(''))
	...
	// > sd '[[:blank:]]' '_' <<< 'hello world'
	// hello_world
	const find = compatRegexpString(args[1] || '')
	const replacements = args.slice(2).map(function (replacement) {
